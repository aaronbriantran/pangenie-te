...
We run all analyses in 3_repeatanno. I:

1. Use bcftools to filter out non-reference insertions in the collective unphased genotypes.
2. Use awk to print the line header as the variant ID and the second line as the variant.
3. Pass that to scripts/repeatmasker.sh, which just runs RepeatMasker.
