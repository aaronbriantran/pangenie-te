...
vg pack -t "${SLURM_CPUS_PER_TASK}" -x "${in_dir}/${1}.giraffe.gbz" -g results/map_reads/F1.gam -o "${out_dir}/${1}.pack" -Q 5
...
vg call -t "${SLURM_CPUS_PER_TASK}" "${in_dir}/${1}.giraffe.gbz" -k "${out_dir}/${1}.pack" -s "${1}" -z > "${out_dir}/${1}_${2}.vcf"
