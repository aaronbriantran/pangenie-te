...
vg pack -t "${SLURM_CPUS_PER_TASK}" -x "${in_dir}/${1}.gbz" -g results/map_reads/F1.gam -o "${out_dir}/${1}.pack" -Q 5
...
vg call -t "${SLURM_CPUS_PER_TASK}" "${in_dir}/${1}.giraffe.gbz" -k "${out_dir}/${1}.pack" -s "${1}" "${VCF_FLAGS[@]}" > "${out_dir}/${1}.vcf"
